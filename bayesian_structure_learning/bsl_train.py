# This code was made publice as part of Deleu et al (2022), UAI
# Their repository is available here: https://github.com/tristandeleu/jax-dag-gflownet
...
def main(args,i):
    ...
    # load the dag and generate new data / get a scorer
    with open("can_output/graph.pkl", "rb") as f:
        graph = pickle.load(f)
    data = sample_from_linear_gaussian(graph, num_samples=100,rng=rng)

    prior = UniformPrior()
    scorer = BGeScore(data=data, prior=prior)
    ...
    data.to_csv(args.output_folder / f'data{i}.csv')
    io.save(args.output_folder / f'model{i}.npz', params=params.online)
...
if __name__ == '__main__':
    ...
    misc.add_argument('--job_index', type=int, required=True,
        help='The job index passed from the shell script (e.g., from an array job).')
